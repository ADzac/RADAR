        ...
        for m = 1:Total_T%Number of Transmitter
            for n = 1:Total_R %Number of Receiver
                ...
                if takeWavelength == 0
                    ...
                    radiation_pattern = abs((-pi/2)*C*sin(phi)*(cos(X)/(X.^2 - (pi/2)^2))*(sin(Y)./Y));
                    radiation_pattern = radiation_pattern*10000;
                    radiation_pattern = (radiation_pattern)/max(radiation_pattern);
                    ...
                    radT = interp1(rad2deg(theta), radiation_pattern, Tangles(m,o), 'nearest', 'extrap');
                    
                    %Receiver
                    angR = acos(abs(newX_R(n,1,1)-X_C(o,1,1))/distancesList_R(n,o));
                    Rangles(n,o) = 90-rad2deg(angR(1));
                    radR = interp1(rad2deg(theta), radiation_pattern, Rangles(n,o),  'nearest', 'extrap');
                    phase = -2i*pi*bande_f(l)/c;
                ...
                elseif radiation == 3
                    S(l,m,n,o) = term*sigma*exp(phase*r)*abs(radT)*abs(radR)/(4*pi*r1*r2);
                end
            end
        end
    end
