...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    m_r = sqrt((epsr*miu_r/2)*(sqrt(1+(power(sigma/(omega*epsr*eps_0),2)))+1));
                    m_i = sqrt((epsr*miu_r/2)*(sqrt(1+(power(sigma/(omega*epsr*eps_0),2)))-1));
                    ...
                    for j = 1:length(theta_SER)
                        u=cos(theta_SER(j));
                        p(:,j)=Mie_S12(index_refraction,x,u);
                        S1(j)= real(p(1,j)'*p(1,j));
                        S2(j)= real(p(2,j)'*p(2,j));
                    end

                    %m1=real(index_refraction); m2=imag(index_refraction);
                    First_term= abs((exp_term/(-1i*k*r)))*cos(phi_SER);
                    Sec_term= abs((exp_term/(1i*k*r)))*sin(phi_SER);

                    E_S_theta = zeros(1,nsteps);
                    E_S_phi = zeros(1,nsteps);
                    E_S_theta = First_term.*S2;
                    E_S_phi= Sec_term.*S1;
                    ...
                    if phi == 0        
                        E = abs(E_S_theta).^2;
                        max_theta = max(E);
                        E_normalized = E/max_theta;
                        E_used = "E_S_\theta";
                    elseif phi == 90
                        E = abs(E_S_phi).^2;
                        max_phi = max(E);
                        E_normalized = E/max_phi;
                        E_used = "E_S_\phi";
                    else
                    ...
                    if (bande_f(l) == fc && fctrue == true)
                       ...
                       % text( 0.5, 0, 'E_S_{Theta}', 'FontSize', 14', 'FontWeight', 'Bold','HorizontalAlignment', 'Center', 'VerticalAlignment', 'Bottom');
                       % 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                elseif radiation == 3
                    S(l,m,n,o) = term*1*exp(phase*r)*abs(radT)*abs(radR)/(4*pi*r1*r2);
                end
            end
        end
    end
