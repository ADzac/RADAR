...
f_min = 0.5e9;
f_max = 1.5e9;
bande = linspace(f_min,f_max,Nf); %If more than 1 frequency
...
N_XVoxels = 150; % Space between each Voxels is L_Xvoxels/N_XVoxels = 0.4
N_YVoxels = 1;
deltaY = 0.2;
...
figure;
plot(Voxels(:,:,1),abs(Image)/max(max(Image)) ,'o', 'MarkerSize', 8, 'MarkerFaceColor', 'b');
