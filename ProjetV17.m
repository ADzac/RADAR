...
theta = round(linspace(-pi/2, pi/2, 10000),4);
phi = linspace(-pi, pi, 20000);% Angles from 0 to 180 degrees

% Define radiation pattern (cosine function for example)
% radiation_pattern = abs(sin(theta));
% radiation_pattern = cos(theta);
%radiation_pattern = sqrt(cos(pi/2*cos(theta)).^2);
%radiation_pattern = (cos(cos(pi/2-theta)*(kmin*l_phy)/2) - cos((kmin*l_phy)/2))./sin(pi/2-theta);
...
% subplot(2,1,2);
% plot(rad2deg(theta), 20*log10(radiation_pattern));
% xlabel('Angle (degrees)');
% ylabel('Radiation ');
% ylim([-60 0]);
...
N_cible = length(X_C(:))/3; %Number of target
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
phi_SER = 0;
SERangles = zeros(Total_T,Total_R);
S1 = zeros(1,length(theta_SER));
S2 = zeros(1,length(theta_SER));
E_S_theta = zeros(size(theta_SER));
E_S_phi = zeros(size(theta_SER));

radius = 0.1; % Sphere radius in meters
miu_r = 1; % Permeability of non-magnetic material
eps_0 = 8.854e-12;
epsr = 4;% Permitivity of the material (4 for dielectric 1 for metallic)
sigma = 1e-6; % Conductivity of the material (10e-6 for dielectric)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Tangles = zeros(Total_T,N_cible);
Rangles = zeros(Total_R,N_cible);
    ...
    for o = 1:N_cible
        for m = 1:Total_T%Number of Transmitter
            for n = 1:Total_R %Number of Receiver
                lambda = c/bande_f(l);
                term = lambda/sqrt(4*pi);
                k = 2*pi/lambda;

                if takeWavelength == 0
                    %Transmitter
                    angT = acos(abs(newX_T(m,1,1)-X_C(o,1,1))/distancesList_T(m,o));
                    Tangles(m,o) = rad2deg(angT(1));
                    radT = interp1(rad2deg(theta), radiation_pattern, Tangles(m,o), 'linear');
                    r1 = distancesList_T(m,o);

                    %Receiver
                    angR = acos(abs(newX_R(n,1,1)-X_C(o,1,1))/distancesList_R(n,o));
                    Rangles(n,o) = rad2deg(angR(1));
                    radR = interp1(rad2deg(theta), radiation_pattern, Rangles(n,o), 'linear');
                    phase = -2i*pi*bande_f(l)/c;
                    r2 = distancesList_R(n,o);
                    ...
                    if phi_SER == 0
                        E2 = abs(E_S_theta).^2;
                        E = sqrt(abs(E_S_theta).^2);
                        E_used = "E_S_\theta";
                    elseif phi_SER == pi/2
                        E2 = abs(E_S_phi).^2;
                        E = sqrt(abs(E_S_phi).^2);
                        E_used = "E_S_\phi";
                    else
                        E = sqrt(abs(E_S_phi).^2 + abs(E_S_theta).^2);
                        E_used = "E_S_\theta and E_S_\phi";
                    end
                    max_E = max(E);
                    E = E/max_E;

                    %Radiation dipole
                    radiation_pattern_dipole = (cos(cos(pi/2-theta)*(k*l_phy)/2) - cos((k*l_phy)/2))./sin(pi/2-theta);
                    radiation_pattern_dipole = radiation_pattern_dipole/max(radiation_pattern_dipole);


                    %Radiation directive
                    X = (k/2)*a*sin(theta)*round(cos(phi),4);
                    Y = (k/2)*b*sin(theta)*sin(phi);
                    term_c = 1i*a*b*k*E0;
                    expo_term_c = exp(-1i*k*r);
                    C = (term_c*expo_term_c)/(2*pi*r);

                    radiation_pattern_directive = abs((-pi/2)*C*sin(phi)*(cos(X)/(X.^2 - (pi/2)^2))*(sin(Y)./Y));
                    radiation_pattern_directive = radiation_pattern_directive*10000;
                    radiation_pattern_directive = (radiation_pattern_directive)/max(radiation_pattern_directive);
                    ...
                    if (bande_f(l) == fc && fctrue == true)
                        ...
                        figure;
                        subplot(2,1,1);
                        plot(rad2deg(theta), (radiation_pattern_directive));
                        xlabel('Angle (degrees)');
                        ylabel('Radiation ');
                        title('Antenna Radiation Pattern Central Frequency');
                        subplot(2,1,2);
                        plot(rad2deg(theta), 10*log10(radiation_pattern_directive));
                        xlabel('Angle (degrees)');
                        ylabel('Radiation ');
                        ylim([-60 0]);
                        title('Antenna Radiation Pattern(dB)');
                        ...
                        figure;
                        subplot(1, 2, 1);
                        polarplot(theta_SER, E);
                        title(['Central Frequency Polar Plot of ' E_used]);

                        subplot(1, 2, 2);
                        plot(rad2deg(theta_SER), 10*log10(E2));
                        hold on;
                        plot(rad2deg(theta_SER), 20*log10(E))
                        title(['Decibel of ' E_used]);
                        xlabel('\phi (degrees)');
                        xlim([0, 360]);
                        legend(E_used + '^2', E_used);

                    elseif (bande_f(l)==f_max && fmaxtrue == true)
                        fmaxtrue = -fmaxtrue;
                        % figure;
                        % subplot(2,1,1);
                        % plot(rad2deg(theta), (radiation_pattern_directive));
                        % xlabel('Angle (degrees)');
                        % ylabel('Radiation ');
                        % title('Antenna Radiation Pattern Max Frequency');
                        % subplot(2,1,2);
                        % plot(rad2deg(theta), 20*log10(radiation_pattern_directive));
                        % xlabel('Angle (degrees)');
                        % ylabel('Radiation ');
                        % ylim([-60 0]);
                        % title('Antenna Radiation Pattern(dB)');
                        ...
                        subplot(1, 2, 1);
                        polarplot(theta_SER, E);
                        title(['Max Frequency Polar Plot of ' E_used]);
                    ...
                    elseif (bande_f(l)==f_min && fmintrue == true)
                        fmintrue = -fmintrue;
                        ...
                        % xlabel('Angle (degrees)');
                        % ylabel('Radiation ');
                        % title('Antenna Radiation Pattern Min Frequency');
                        % subplot(2,1,2);
                        % plot(rad2deg(theta), 20*log10(radiation_pattern_directive));
                        % xlabel('Angle (degrees)');
                        % ylabel('Radiation ');
                        ...
                        subplot(1, 2, 1);
                        polarplot(theta_SER, E);
                        title(['Min Frequency Polar Plot of ' E_used]);
                    ...
                    %Transmitter
                    angT = acos(abs(newX_T(m,1,1)-X_C(o,1,1))/distancesList_T(m,o));
                    Tangles(m,o) = 90-rad2deg(angT(1));
                    radT = interp1(rad2deg(theta), radiation_pattern_dipole, Tangles(m,o), 'nearest', 'extrap');
                    
                    %Receiver
                    angR = acos(abs(newX_R(n,1,1)-X_C(o,1,1))/distancesList_R(n,o));
                    Rangles(n,o) = 90-rad2deg(angR(1));
                    radR = interp1(rad2deg(theta), radiation_pattern_dipole, Rangles(n,o),  'nearest', 'extrap');

                    SERangles(m,n) = abs(180 - (abs(Tangles(m,o)-Rangles(n,o))));
                    sigma_SER = interp1(rad2deg(theta_SER), E, SERangles(m,n),  'nearest', 'extrap');
                    phase = -2i*pi*bande_f(l)/c;
                ...
                if radiation == 0
                    S(l,m,n,o) = exp(phase*r);
                elseif radiation == 1
                    S(l,m,n,o) = exp(phase*r)/(4*pi*r1*4*pi*r2);
                elseif radiation == 2
                    S(l,m,n,o) = exp(phase*r)*abs(radT)*abs(radR);
                elseif radiation == 3
                    S(l,m,n,o) = term*sigma_SER*exp(phase*r)*abs(radT)*abs(radR)/(4*pi*r1*r2);
                end
            end
        end
    end
...
m1 = abs(max(max(max(max(S(:,:,:,1))))));

if N_cible > 1
    m2 = abs(max(max(max(max(S(:,:,:,2))))));
    gain = m1/m2;

    if m1 > m2
        disp('Test field for 1st target is higher');
        disp(gain);
        disp('First target')
        disp(string(X_C(1,1,1)) + " "  + string(X_C(1,1,2)) + " " +  string(X_C(1,1,3)));
        disp('2nd target')
        disp(string(X_C(2,1,1)) + " "  + string(X_C(2,1,2)) + " " +  string(X_C(2,1,3)));

    else
        disp('Test field for 2nd target is higher');
        disp('First target')
        disp(string(X_C(1,1,1)) + " "  + string(X_C(1,1,2)) + " " +  string(X_C(1,1,3)));
        disp('2nd target')
        disp(string(X_C(2,1,1)) + " "  + string(X_C(2,1,2)) + " " +  string(X_C(2,1,3)));
...
if radiation == 0
    save('data.mat', 'TotalImage_YCTE','TotalImage_XCTE','TotalImage_ZCTE','TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','X1','Z1');
elseif radiation == 1
    save('data_rad1.mat','TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE', 'TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','X1','Z1');
elseif radiation == 2
    save('data_rad2.mat', 'TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE','TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','X1','Z1');
elseif radiation == 3
    save('data_rad3.mat','TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE', 'TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','X1','Z1');
end
...
% clim([0 1]);
%
% dB = 20*log10(TotalImageReshaped_XCTE_Normalize);
%
% subplot(1,2,2);
% surf(Y3, Z3, dB, 'EdgeColor', 'none');
% shading("interp");
% xlabel('Z-axis');
% ylabel('Y-axis');
% title('3D Image Plot with X cte = ' + string(xVoxels_coord) + ' of target on  [' + string(X_C(:,1)) + ',' + string(X_C(:,2)) + ',' + string(X_C(:,3)) + ']' );
...
% % Save data to MAT file
% if radiation == 0
%     save('data.mat', 'TotalImage_YCTE','TotalImage_XCTE','TotalImage_ZCTE','TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','Y3','Z3');
% elseif radiation == 1
%     save('data_rad1.mat','TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE', 'TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','Y3','Z3');
% elseif radiation == 2
%     save('data_rad2.mat', 'TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE','TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','Y3','Z3');
% elseif radiation == 3
%     save('data_rad3.mat','TotalImage_YCTE','TotalImage_ZCTE','TotalImage_XCTE', 'TotalImageReshaped_YCTE', 'TotalImageReshaped_ZCTE', 'TotalImageReshaped_XCTE','BigMax','Y3','Z3');
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
