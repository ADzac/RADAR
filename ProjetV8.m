...
T = 1; % 1 if T 2 if X
%Numbers of antennas
N_Rx = 2; %Receiver on x-axis
N_Ry = 2; %Receiver on y-axis
% put 1 in N_Tx and N_Ty for T / X structure
N_Tx = 6;
N_Ty = 1; 

if T ==1 || T==2
    numberOfnewT = 3; % note that 1 will create two at the end of each line
end
...
%Additional antennas for T shaped structure
    ...
    maxR = max(max(X_R(:,:,2)));
    minR = min(min(X_R(:,:,2)));
    for i=1:numberOfnewT
        %highest point
        maxR = maxR+shannon/2;
        X_T(newT(1)+i,newT(2)+1,1) = yT_coord;
        X_T(newT(1)+i,newT(2)+1,2) = maxR;
        X_T(newT(1)+i,newT(2)+1,3) = z_coord;
        %lowest point
        minR= minR-shannon/2;
        X_T(newT(1)+i+1*numberOfnewT,newT(2)+1,1) = yT_coord;
        X_T(newT(1)+i+1*numberOfnewT,newT(2)+1,2) = minR;
        X_T(newT(1)+i+1*numberOfnewT,newT(2)+1,3) = z_coord;
    end
    newT = size(X_T);
...
figure;
subplot(1,2,1);
contourf(X1, Z1,TotalImageReshaped_YCTE_Normalize, 'EdgeColor', 'none');
xlabel('Z-axis');
ylabel('X-axis');
...
subplot(1,2,2);
contourf(X1, Z1, 20*log10(TotalImageReshaped_YCTE_Normalize), 'EdgeColor', 'none');
xlabel('Z-axis');
ylabel('X-axis');
colorbar;
caxis([-30 0]);
...
figure;
subplot(1,2,1);
contourf(X2, Y2, TotalImageReshaped_ZCTE_Normalize, 'EdgeColor', 'none');
xlabel('Y-axis');
ylabel('X-axis');
...
subplot(1,2,2);
contourf(X2, Y2, 20*log10(TotalImageReshaped_ZCTE_Normalize), 'EdgeColor', 'none');
xlabel('Y-axis');
ylabel('X-axis');
colorbar;
caxis([-30 0]);
...
figure;
subplot(1,2,1);
contourf(Y3, Z3, TotalImageReshaped_XCTE_Normalize , 'EdgeColor', 'none');
xlabel('Z-axis');
ylabel('Y-axis');
...
subplot(1,2,2);
contourf(Y3, Z3, 20*log10(TotalImageReshaped_XCTE_Normalize ), 'EdgeColor', 'none');
xlabel('Z-axis');
ylabel('Y-axis');
colorbar;
caxis([-30 0]);
