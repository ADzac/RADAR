...
T = 0; % 1 if T 2 if X 3 if square shape at the outer of receivers square
%Numbers of antennas
...
plot3(X_C(:,1), X_C(:,2), X_C(:,3), 'o', 'MarkerSize', 8, 'MarkerFaceColor', 'b');
hold;

plot3(X_T(:,:,1), X_T(:,:,2), X_T(:,:,3), 'o', 'MarkerSize', 8, 'MarkerFaceColor', 'g');
plot3(X_R(:,:,1), X_R(:,:,2), X_R(:,:,3), 'o', 'MarkerSize', 8, 'MarkerFaceColor', 'r');
%plot3(Voxels1(:,:,1), Voxels1(:,:,2), Voxels1(:,:,3), 'o', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
% Add labels and title
xlabel('X-axis');
ylabel('Y-axis');
zlabel('Z-axis');
legend('Target(s)','Transmitter', 'Receiver', 'Location','Northeast');
