...
%radiation_pattern = (cos(cos(pi/2-theta)*(kmin*l_phy)/2) -
%cos((kmin*l_phy)/2))./sin(pi/2-theta);
        ...
        for m = 1:Total_T%Number of Transmitter
            for n = 1:Total_R %Number of Receiver
                ...
                if takeWavelength == 0
                    ...
                    %Radiation
                    radiation_pattern = (cos(cos(pi/2-theta)*(k*l_phy)/2) - cos((k*l_phy)/2))./sin(pi/2-theta);
                ...
                elseif radiation == 3
                    S(l,m,n,o) = term*sigma*exp(phase*r)*sqrt(abs(radT))*sqrt(abs(radR))/(4*pi*r1*r2);
                end
            end
        end
    end
