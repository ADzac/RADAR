...
%Numbers of antennas
N_Rx = 8; %Receiver on x-axis
N_Ry = 8; %Receiver on y-axis
N_Tx = 1;
N_Ty = 1;
        ...
        for m=1:N_ZVoxels
            for n = 1:N_Tx*N_Ty %Number of Transmitter
                for o = 1:N_Rx*N_Ry %Number of Receiver
                    Image_YCTE(k,l,m) = Image_YCTE(k,l,m) + S(k,n,o)*exp(((2i*pi*bande_f(k))/c)*(distancesList_TV1(l,m,n)+distancesList_RV1(l,m,o))); %bande_f(l)
                end
            end
        end
    end
...
for k = 1:Nf
    for l = 1:N_XVoxels
        for m=1:N_YVoxels
            for n = 1:N_Tx*N_Ty %Number of Transmitter
                for o = 1:N_Rx*N_Ry %Number of Receiver
                    Image_ZCTE(k,l,m) = Image_ZCTE(k,l,m) + S(k,n,o)*exp(((2i*pi*bande_f(k))/c)*(distancesList_TV2(l,m,n)+distancesList_RV2(l,m,o))); 
                end
            end
        end
    end
...
for k = 1:Nf
    for l = 1:N_YVoxels
        for m=1:N_ZVoxels
            for n = 1:N_Tx*N_Ty %Number of Transmitter
                for o = 1:N_Rx*N_Ry %Number of Receiver
                        Image_XCTE(k,l,m) = Image_XCTE(k,l,m) + S(k,n,o)*exp(((2i*pi*bande_f(k))/c)*(distancesList_TV3(l,m,n)+distancesList_RV3(l,m,o))); %bande_f(l)
